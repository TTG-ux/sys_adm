...
1. Выполняем команду `cron -e`, открыыается cron с закомментированными строками, спускаемся вниз и пишем следующие строки:
...
-   Из своего опыта могу сказать, что логирования через службу `timesyncd` работает, но не логирует так как нужно крупным компаниям, можно смотреть логи системы, но из-за обилия мусора системных логов, банально тяжело будет и очень ресурсно-затратно это делать.
